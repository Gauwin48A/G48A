...
const X = () => {
  const { t: translate } = W(),
    ...
    fetchPost = $(
      ...
      [apiBase, postId, locationPost, retryCount],
    );
  y(() => {
    ...
    y(() => {
      if (!postId) return;
      fetch(`${apiBase}/api/posts/${postId}/view`, {
        method: "POST",
        credentials: "include",
      }).catch(() => {});
      const t =
      ...
      t &&
        fetch(`${apiBase}/api/recently-viewed/track`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ postId: postId, userId: t, source: "feed" }),
        }).catch(() => {});
    }, [apiBase, postId]);
  const handleLike = async () => {
      if (postId) {
        ...
        try {
          await fetch(`${apiBase}/api/posts/${postId}/like`, {
            method: "POST",
            credentials: "include",
          });
        } catch {}
