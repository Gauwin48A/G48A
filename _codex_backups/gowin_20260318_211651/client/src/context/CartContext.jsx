...
export function CartProvider({ children }) {
  ...
  const addItem = (item) => {
    ...
    setItems((prev) => {
      ...
      // Increment qty on duplicate tap, capped at MAX_QTY.
      const next = [...prev];
      const currentQty = Number(next[index].qty ?? 1);
      next[index] = { ...next[index], qty: Math.min(currentQty + 1, MAX_QTY) };
      return next;
    });
  };
  ...
  const updateQty = (id, qty) => {
    const targetId = String(id ?? "");
    if (!targetId) return;
    const nextQty = Math.min(MAX_QTY, Math.max(0, Number(qty) || 0));
    setItems((prev) => {
