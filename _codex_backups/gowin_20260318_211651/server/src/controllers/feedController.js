...
const { CATEGORY_GROUP_VALUES } = require("../utils/categoryGroupSql");
 ...
 * @returns {{ page: number, limit: number, offset: number, sortBy: string, sortOrder: string, search: string|null, category: string|null }}
 */
...
exports.getNearbyFeed = async (req, res) => {
  try {
    ...
    const radiusKm = parsePositiveNumber(radius, 10, 100);
    ...
    if (
      ...
      return res.status(501).json({
        ...
        message:
          "Please run the production hardening migration to enable location-based search.",
  ...
  try {
    ...
    try {
      result = await queryWithTimeout(
        ...
        ORDER BY
          CASE WHEN COALESCE(u.current_plan,'basic') = 'premium' AND COALESCE(p.boost_level,0) >= 2 THEN 1
               WHEN COALESCE(u.current_plan,'basic') = 'premium' THEN 2
               WHEN COALESCE(u.current_plan,'basic') = 'silver' AND COALESCE(p.boost_level,0) >= 2 THEN 3
               WHEN COALESCE(u.current_plan,'basic') = 'silver' THEN 4
               WHEN COALESCE(u.current_plan,'basic') = 'bronze' THEN 5
               ELSE 6
          END ASC,
