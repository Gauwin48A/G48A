    ...
    const fetchSoldPosts = async () => {
      ...
      try {
        const response = await api.get('/posts/mine', {
          params: { userId, status: 'sold', limit: 100, page: 1, sortBy: 'updated_at', sortOrder: 'desc' },
        });
        if (cancelled) return;
        const payload = response?.data ?? response;
        const sourcePosts = Array.isArray(payload?.posts) ? payload.posts : [];
        const soldOwnPosts = sourcePosts.filter((post) => String(post?.ownership || 'own').toLowerCase() !== 'bought');
        setPosts(soldOwnPosts);
      } catch (err) {
        if (!cancelled) {
          setError(err.message || t('error') || 'Failed to load sold posts');
          setPosts([]);
        }
      } finally {
        if (!cancelled) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900 px-4">
        <div className="text-center max-w-md">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">{t('sold_posts') || 'Sold Posts'}</h2>
          <p className="text-gray-500 dark:text-gray-400 mb-6">{t('please_login_view') || 'Please log in to view your sold posts.'}</p>
    ...
    <div className="bg-white dark:bg-gray-900 min-h-screen flex flex-col items-center transition-colors duration-300 pb-24">
      <div className="w-full max-w-2xl mx-auto py-6 px-4">
        <div className="flex items-center justify-between gap-2 mb-4">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('sold_posts') || 'Sold Posts'}</h2>
        ...
        {loading ? (
          <div className="space-y-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={`sold-skeleton-${index}`} className="rounded-xl border border-gray-200 dark:border-gray-700 p-4 animate-pulse">
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-900 p-6 text-center">
            <p className="text-blue-900 dark:text-blue-200 font-semibold mb-2">No sold posts yet</p>
