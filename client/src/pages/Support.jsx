...
import { useCoinBalance, useClaimCheckIn, useStoreRedeem } from "@/hooks/useCoins";
...
const normalizeSupportArticles = (content, fallback) => {
  const source =
    ...
    ? raw.map((entry, index) => {
        ...
        return {
          id:
            ...
            entry.key ||
            entry.slug ||
            `support-article-${index + 1}`,
          titleKey: entry.titleKey || entry.title_key || "",
          titleFallback: entry.titleFallback || entry.title || entry.name || "",
          summaryKey: entry.summaryKey || entry.summary_key || "",
          summaryFallback:
            entry.summaryFallback ||
            entry.summary ||
            entry.description ||
            "",
          tagKey: entry.tagKey || entry.tag_key || "",
          tagFallback: entry.tagFallback || entry.tag || entry.category || "",
          readTimeMinutes:
            Number(entry.readTimeMinutes || entry.read_time_minutes || entry.readTime || 0) ||
            4,
        };
      })
    : [];
  const normalized = items.filter((entry) => entry && entry.titleFallback);
  return normalized.length ? normalized : fallback;
};

const normalizeSupportModules = (content, fallback) => {
  const source =
    content && typeof content === "object" && !Array.isArray(content)
      ? content
      : {};
  ...
  const items = Array.isArray(raw)
    ? raw.map((entry, index) => {
        if (!entry || typeof entry !== "object") return null;
        return {
          id: entry.id || entry.key || entry.slug || `support-module-${index + 1}`,
          titleKey: entry.titleKey || entry.title_key || "",
          titleFallback: entry.titleFallback || entry.title || entry.name || "",
          descriptionKey: entry.descriptionKey || entry.description_key || "",
          descriptionFallback:
            entry.descriptionFallback ||
            entry.description ||
            entry.summary ||
            "",
          cost: Number(entry.cost || entry.price || entry.coins || 0) || 0,
        };
      })
    : [];
  const normalized = items.filter((entry) => entry && entry.titleFallback);
...
export default function Support() {
  ...
  const { data: cmsContent } = useCmsPage("support");
  ...
  const { data: coinBalance = 0, refetch: refetchBalance } = useCoinBalance({ enabled: loggedIn });
  const claimCheckIn = useClaimCheckIn();
  const storeRedeem = useStoreRedeem();
  const wallet = useMemo(
    () => (loggedIn ? { balance: coinBalance } : null),
    [loggedIn, coinBalance],
  );

  // Fetch recent transactions from API
  useEffect(() => {
    if (!loggedIn) { setTransactions([]); return; }
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!cancelled) setTransactions(Array.isArray(data?.history) ? data.history : Array.isArray(data?.transactions) ? data.transactions : []);
      } catch { if (!cancelled) setTransactions([]); }
    })();
    return () => { cancelled = true; };
  }, [loggedIn, coinBalance]);

  const supportArticles = useMemo(
    () => normalizeSupportArticles(cmsContent, DISEASE_ARTICLES),
    [cmsContent],
  );
  const supportModules = useMemo(
    () => normalizeSupportModules(cmsContent, PREMIUM_MODULES),
    [cmsContent],
  );

  const visibleLibraryItems = supportArticles.slice(0, visibleArticles);
  const hasMoreArticles = visibleArticles < supportArticles.length;
  ...
  const getModuleTitle = useCallback(
    (moduleId) => {
      const module = supportModules.find((entry) => entry.id === moduleId);
      if (!module) {
        return moduleId;
      }
  ...
  const handleClaimDaily = async () => {
    ...
    try {
      const result = await claimCheckIn.mutateAsync();
      const amount = result?.coinsEarned || 20;
      setBannerMessage({
      ...
      if (msg.toLowerCase().includes("already")) {
        ...
        setBannerMessage({
          key: "support_daily_claim_failed",
          fallback: "Could not claim daily reward right now.",
        });
      }
    }
  };

  const handleUnlockModule = async (module) => {
    if (!loggedIn) {
      requireLogin();
      return;
    }
    ...
    try {
      await storeRedeem.mutateAsync({ type: module.id });
      setBannerMessage({
      ...
      const msg = err?.response?.data?.message || err?.message || "";
      if (msg.toLowerCase().includes("not enough") || msg.toLowerCase().includes("insufficient")) {
        setBannerMessage({
  ...
  return (
    <div className="min-h-screen mhub-premium-page bg-gradient-to-b from-slate-50 via-white to-slate-100 dark:bg-gradient-to-b">
      <div className="mx-auto w-full max-w-6xl px-4 pt-6 md:px-6 md:pt-8">
        <div className="mb-6 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-white shadow-lg md:p-6 dark:border dark:border-blue-600/40 dark:bg-gradient-to-r dark:text-white">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-2xl font-bold md:text-3xl dark:text-2xl dark:md:text-3xl">
                {tr("support_page_title", "Medication & Support")}
              </h1>
              <p className="mt-1 text-sm text-blue-100 md:text-base dark:text-sm dark:text-blue-200 dark:md:text-base">
                {tr(
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-sm font-semibold dark:bg-slate-900/15 dark:text-sm">
        ...
        {resolvedBanner ? (
          <div className="mb-5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-200 dark:border dark:border-blue-600/40 dark:bg-blue-950/20 dark:text-sm">
            {resolvedBanner}
          </div>
        ) : null}

        <div className="mb-6 grid gap-4 md:grid-cols-3">
          <Card className="border-blue-100 dark:border-gray-700 md:col-span-2 dark:border-blue-600/40">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg dark:text-lg">
            ...
            <CardContent className="space-y-3">
              {!loggedIn ? (
                <div className="rounded-xl border border-dashed border-blue-200 bg-blue-50/60 p-4 text-sm text-blue-800 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200 dark:border dark:border-dashed dark:border-blue-600/40 dark:bg-blue-950/60 dark:text-sm">
                  ...
                  <Button
                    type="button"
                    className="mt-3 bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-700/40 dark:text-white dark:hover:bg-blue-700/40"
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/25 sm:flex-row sm:items-center sm:justify-between dark:border dark:border-amber-600/40 dark:bg-amber-950/20">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300 dark:text-xs">
                        {tr("support_available_balance", "Available Balance")}
                      </p>
                      <p className="text-2xl font-bold text-amber-900 dark:text-amber-200 dark:text-2xl">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      className="border-amber-300 bg-white text-amber-800 hover:bg-amber-100 dark:border-amber-700 dark:bg-transparent dark:text-amber-200 dark:border-amber-600/40 dark:bg-slate-900 dark:hover:bg-amber-950/20"
                  ...
                  <div className="space-y-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 dark:text-xs dark:text-gray-300">
                    ...
                    {transactions.length === 0 ? (
                      ...
                      transactions.map((entry) => (
                        <div
                          ...
                          className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 text-sm dark:border-gray-700 dark:border dark:text-sm"
                        >
                          <div>
                            <p className="font-medium text-gray-800 dark:text-gray-200 dark:text-gray-100">
          ...
          <Card className="border-blue-100 dark:border-gray-700 dark:border-blue-600/40">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg dark:text-lg">
            ...
            <CardContent className="space-y-3 text-sm text-gray-600 dark:text-gray-300 dark:text-sm dark:text-gray-200">
              <p>
                {tr(
              ...
              <div className="rounded-lg border border-dashed border-blue-200 bg-blue-50 px-3 py-2 text-blue-700 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200 dark:border dark:border-dashed dark:border-blue-600/40 dark:bg-blue-950/20 dark:text-blue-300">
                {tr("login_to_continue", "Login to continue")}
              </div>
            </CardContent>
          </Card>
        ...
        <div className="mb-8">
          <div className="mb-3 flex items-center gap-2">
            <HeartPulse className="h-5 w-5 text-blue-600 dark:text-blue-300" />
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 dark:text-xl">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {supportModules.map((module) => {
              ...
              return (
                <Card
                  key={module.id}
                  className="border-blue-100 dark:border-gray-700 dark:border-blue-600/40"
                >
                  <CardHeader className="space-y-2 pb-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {unlocked ? (
                        <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-300 dark:hover:bg-emerald-950/20">
                          {tr("support_unlocked", "Unlocked")}
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="border-blue-200 dark:border-blue-600/40">
                  ...
                  <CardContent>
                    {!loggedIn ? (
                      <div className="space-y-3 rounded-xl border border-dashed border-blue-200 bg-blue-50 p-3 dark:border-blue-900 dark:bg-blue-950/30 dark:border dark:border-dashed dark:border-blue-600/40 dark:bg-blue-950/20">
                        <p className="text-sm font-semibold text-blue-800 dark:text-blue-200 dark:text-sm">
                        ...
                        <Button
                          type="button"
                          size="sm"
                          className="w-full bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-700/40 dark:text-white dark:hover:bg-blue-700/40"
                          onClick={requireLogin}
                        >
                          {tr("login_to_continue", "Login to continue")}
                        </Button>
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        className="w-full border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:border-emerald-600/40 dark:bg-emerald-950/20 dark:text-emerald-300 dark:hover:bg-emerald-950/20"
                        disabled
                      >
                      ...
                      <div className="space-y-2">
                        ...
                        {!canUnlock ? (
                          <p className="text-xs text-rose-600 dark:text-rose-400 dark:text-xs dark:text-rose-300">
                            {tr(
          ...
          {hasMoreArticles ? (
            <div className="mt-5 flex justify-center">
              <Button
                type="button"
                variant="outline"
                className="border-blue-200 text-blue-700 hover:bg-blue-50 dark:border-blue-600/40 dark:text-blue-300 dark:hover:bg-blue-950/20"
                onClick={() =>
        ...
        <Card className="border-blue-100 dark:border-gray-700 dark:border-blue-600/40">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg dark:text-lg">
          ...
          <CardContent className="grid gap-2 md:grid-cols-2">
            <Link
              to="/t&c"
              className="flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800 dark:border dark:text-sm dark:hover:bg-gray-950"
            >
