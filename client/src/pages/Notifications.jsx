...
import b from "../lib/api";
import { useAuth as K } from "@/context/AuthContext";
import { getUserId as V, isAuthenticated as J } from "@/utils/authStorage";
import {
  Bell as S,
  Check as O,
  Trash2 as X,
...
import me from "../components/PageHeader";
const pe = [
    "/all-posts",
    "/post/",
    "/bought-posts",
  ...
  ge = (s) => {
    ...
    const r = String(s).trim().startsWith("/")
      ? String(s).trim()
      : `/${String(s).trim()}`;
  ...
  he = (s) => {
    const r = String(s.icon || s.icon_category || s.type || "").toLowerCase();
    return r.includes("order") || r.includes("sale") || r.includes("package")
      ? "package"
      : r.includes("payment") || r.includes("wallet") || r.includes("money")
        ? "money"
        : r.includes("security")
          ? "security"
          : r.includes("message") || r.includes("chat") || r.includes("inquiry")
            ? "message"
            : r.includes("gift") || r.includes("promo") || r.includes("reward")
              ? "gift"
              : r.includes("heart") || r.includes("like")
                ? "heart"
                : r.includes("star") ||
                    r.includes("subscription") ||
                    r.includes("tier")
                  ? "star"
                  : r.includes("trend") || r.includes("price")
                    ? "trending"
                    : r.includes("alert") || r.includes("warning")
                      ? "alert"
                      : "package";
  },
  C = (s) => {
    const r = s.notification_id ?? s.id;
    return {
      ...s,
      id: r,
      notification_id: r,
      read: s.read === !0 || s.is_read === !0,
      icon: he(s),
  ...
  fe = () => {
    const s = G(),
      ...
      k = g(async () => {
        ...
        try {
          const a = await b.get(
            `/notifications?userId=${encodeURIComponent(o)}`,
          );
          ...
          const i = a?.data ?? a,
            l = Array.isArray(i)
              ...
              : Array.isArray(i?.notifications)
                ? i.notifications
                : [];
          l.length > 0 ? u(l.map(C)) : u(N.map(C));
        } catch (a) {
          if (t !== p.current) return;
          console.error("[Notifications] Failed to load notifications:", a),
            u(N.map(C));
        } finally {
          t === p.current && v(!1);
        }
    ...
    const E = (t) => {
        ...
        switch (t) {
          case "package":
            return e.createElement(Z, { className: `${a} text-blue-500` });
          case "money":
            return e.createElement(Q, { className: `${a} text-emerald-500` });
          case "gift":
            return e.createElement(ie, { className: `${a} text-purple-500` });
          case "message":
            return e.createElement(re, { className: `${a} text-indigo-500` });
          case "security":
            return e.createElement(R, { className: `${a} text-orange-500` });
          case "heart":
            return e.createElement(ee, { className: `${a} text-pink-500` });
          case "star":
            return e.createElement(ne, { className: `${a} text-yellow-500` });
          case "trending":
            return e.createElement(oe, { className: `${a} text-green-500` });
          case "alert":
            return e.createElement(te, { className: `${a} text-red-500` });
          default:
            return e.createElement(S, { className: `${a} text-blue-500` });
        }
      },
      W = (t, a) => {
        const i = a ? "20" : "30";
        switch (t) {
          case "package":
            return `bg-gradient-to-br from-blue-500/${i} to-blue-600/${i}`;
          case "money":
            return `bg-gradient-to-br from-emerald-500/${i} to-emerald-600/${i}`;
          case "gift":
            return `bg-gradient-to-br from-purple-500/${i} to-pink-500/${i}`;
          case "message":
            return `bg-gradient-to-br from-indigo-500/${i} to-blue-500/${i}`;
          case "security":
            return `bg-gradient-to-br from-orange-500/${i} to-red-500/${i}`;
          case "heart":
            return `bg-gradient-to-br from-pink-500/${i} to-rose-500/${i}`;
          case "star":
            return `bg-gradient-to-br from-yellow-500/${i} to-amber-500/${i}`;
          case "trending":
            return `bg-gradient-to-br from-green-500/${i} to-emerald-500/${i}`;
          default:
            return `bg-gradient-to-br from-blue-500/${i} to-indigo-500/${i}`;
        }
      },
      F = (t) => {
        const a = new Date(t),
      ...
      _ = g(
        async (t) => {
          u((a) =>
            a.map((i) =>
              (i.notification_id || i.id) === t
                ? { ...i, read: !0, is_read: !0 }
                : i,
            ),
          );
          try {
            await b.put(`/notifications/${t}/read`, { userId: o });
          } catch (a) {
            console.error(
      ...
      j = g(async () => {
        u((t) => t.map((a) => ({ ...a, read: !0, is_read: !0 })));
        try {
          await b.put("/notifications/mark-all-read", { userId: o });
        } catch (t) {
          console.error("[Notifications] Failed to mark all as read:", t);
        }
      }, [o]),
      q = g(
        async (t) => {
          u((a) => a.filter((i) => (i.notification_id || i.id) !== t)),
            D((a) => {
              const i = new Set(a);
              return i.delete(t), i;
            });
          try {
            await b.delete(`/notifications/${t}`, { data: { userId: o } });
          } catch (a) {
            console.error("[Notifications] Failed to delete notification:", a);
          }
        },
        [o],
      ),
      U = g(async () => {
        ...
        t.length !== 0 &&
          (u((a) => a.filter((i) => !x.has(i.notification_id || i.id))),
          D(new Set()),
          await Promise.allSettled(
            t.map((a) =>
              b.delete(`/notifications/${a}`, { data: { userId: o } }),
            ),
          ));
      }, [x, o]),
      m = f(() => n.reduce((t, a) => t + (a.read ? 0 : 1), 0), [n]),
      Y = g(
        (t) => {
          const a = t.notification_id || t.id,
            i = t?.action?.path || "",
            l = ge(i);
          _(a),
            s(l, {
              state:
                l !== i
                  ? { fromNotificationFallback: !0, originalPath: i }
                  : void 0,
            });
        },
        [_, s],
      ...
      I = f(
        () =>
          n.filter((t) =>
            c === "unread" ? !t.read : c === "read" ? t.read : !0,
          ),
        [c, n],
      );
    return d
      ...
      : !y || !o
        ? e.createElement(
            ...
            e.createElement(
              ...
              e.createElement(
                ...
                r("login_required") || "Login Required",
              ),
              e.createElement(
                "p",
                { className: "text-gray-300 mb-6" },
                r("please_login_to_continue") ||
              ...
              e.createElement(
                ...
                {
                  onClick: () =>
                    s("/login", { state: { returnTo: "/notifications" } }),
                  className:
        ...
        : L
          ...
          : e.createElement(
              ...
              e.createElement(me, {
                title: r("notifications") || "Notifications",
                rightAction: e.createElement(
                  ...
                  x.size > 0 &&
                    e.createElement(
                      ...
                      e.createElement(X, { className: "w-5 h-5 text-red-400" }),
                    ),
                  e.createElement(
                    "button",
                    {
              ...
              e.createElement(
                ...
                e.createElement(
                  ...
                  e.createElement(
                    ...
                    m > 0
                      ? `${m} ${r("new_updates") || "new updates"}`
                      : r("all_caught_up") || "You're all caught up!",
                  ),
                  m > 0 &&
                    e.createElement(
                      "button",
                      ...
                      r("mark_all_as_read") || "Mark all as read",
                    ),
                  e.createElement(
                    "div",
                    {
                    ...
                    [
                      { key: "all", label: "All", count: n.length, icon: T },
                      { key: "unread", label: "Unread", count: m, icon: le },
                      {
                        key: "read",
                        label: "Read",
                        count: n.length - m,
                      ...
                      e.createElement(
                        "button",
                        {
                          key: t.key,
                          onClick: () => M(t.key),
                          className: `flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm whitespace-nowrap transition-all ${c === t.key ? "bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg shadow-purple-500/30" : "bg-white/5 text-purple-200 hover:bg-white/10 border border-white/10"}`,
                        },
                        e.createElement(t.icon, { className: "w-4 h-4" }),
                        t.label,
                        e.createElement(
                          "span",
                          {
                            className: `px-1.5 py-0.5 rounded-md text-xs ${c === t.key ? "bg-white/20" : "bg-white/10"}`,
                          },
                          t.count,
                        ),
                      ),
                  ...
                  I.length === 0
                    ? e.createElement(
                        ...
                        e.createElement(
                          ...
                          c === "unread"
                            ? r("all_caught_up") || "All caught up!"
                            : r("no_notifications") || "No notifications",
                        ),
                        e.createElement(
                          "p",
                          { className: "text-purple-300/70" },
                          c === "unread"
                            ? r("read_all_notifications") ||
                    ...
                    : I.map((t, a) =>
                        e.createElement(
                          "div",
                          {
                            key: t.notification_id || t.id,
                            className: `group relative overflow-hidden rounded-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-xl ${t.read ? "bg-white/5 border border-white/10 hover:border-white/20" : "bg-gradient-to-r from-white/10 to-white/5 border border-purple-500/30 shadow-lg shadow-purple-500/10"}`,
                          ...
                          e.createElement(
                            ...
                            e.createElement(
                              "div",
                              {
                                className: `w-12 h-12 rounded-2xl ${W(t.icon, t.read)} flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110`,
                              },
                              E(t.icon),
                            ),
                            e.createElement(
                              ...
                              e.createElement(
                                ...
                                e.createElement(
                                  "h3",
                                  {
                                    className: `font-semibold text-white truncate flex-1 min-w-0 ${t.read ? "text-gray-200" : "text-white"}`,
                                    title: t.title,
                                  },
                                  t.title,
                                ),
