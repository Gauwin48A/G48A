...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = mapError(status, errData.error || errData.message || err.message);
      setErrorMessage(msg);
      toast({ title: t("error") || "Error", description: msg, variant: "destructive" });
    } finally {
      setLoading(false);
        ...
        <Card className="max-w-md w-full shadow-xl border-0 rounded-2xl sm:rounded-3xl mhub-premium-surface dark:border-0">
          <CardContent className="p-6 sm:p-8 text-center">
            ...
            <Link to="/forgot-password">
              <Button className="w-full h-11 sm:h-12 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white rounded-xl font-semibold dark:bg-gradient-to-r dark:text-white">
