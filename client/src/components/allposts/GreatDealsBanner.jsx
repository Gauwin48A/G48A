        ...
        {collapsed ? (
          <div className={`great-deals-banner-panel flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 rounded-2xl border border-slate-200/70 dark:border-slate-800/60 bg-white/95 dark:bg-slate-900/60 shadow-sm backdrop-blur-sm ${collapsedClass}`}>
            <div className="flex flex-col gap-0.5">
              <span className="inline-flex items-center gap-1.5 text-[11px] uppercase tracking-wide font-semibold text-slate-600">
              ...
              <span className={`${compact ? "text-[9px]" : "text-[11px]"} text-slate-600 dark:text-slate-300`}>
                {contextLabel}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              {allowCollapse ? (
                <button
                  type="button"
                  className={`inline-flex items-center justify-center rounded-full border border-slate-200 bg-white ${compact ? "px-2.5 py-0.5 text-[10px]" : "px-3 py-1 text-[11px]"} font-semibold text-slate-700 hover:bg-slate-50 dark:bg-slate-900 dark:text-slate-200 dark:border-slate-700 dark:hover:bg-slate-800`}
                  onClick={onExpand}
                >
                  {text("expand", "Expand")}
                </button>
              ...
              <button
                type="button"
                className={`inline-flex items-center justify-center rounded-full bg-blue-600 ${compact ? "px-2.5 py-0.5 text-[10px]" : "px-3 py-1 text-[11px]"} font-semibold text-white hover:bg-blue-700`}
                onClick={onShopNow}
              >
          ...
          <>
            <div className={`great-deals-banner-panel sm:hidden relative flex items-center justify-between gap-3 rounded-2xl border border-slate-200/70 dark:border-slate-800/60 bg-gradient-to-r from-blue-50 via-white to-sky-50 dark:from-slate-900/70 dark:via-slate-900/60 dark:to-slate-900/70 shadow-sm ${compact ? "px-2.5 py-1.5" : "px-3 py-2.5"}`}>
              <div className="flex flex-col gap-1">
                ...
                <span className={`${compact ? "text-[9px]" : "text-[11px]"} text-blue-700 dark:text-blue-300`}>
                  {subtitle}
                </span>
                <span className={`${compact ? "text-[9px]" : "text-[11px]"} text-blue-700/90 dark:text-blue-300`}>
                  {contextLabel}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className={`bg-blue-600 text-white font-semibold ${compact ? "px-2.5 py-1 text-[10px]" : "px-3 py-1.5 text-[11px]"} rounded-full inline-flex items-center gap-1.5`}
                  onClick={onShopNow}
                >
                  <FaBolt className={compact ? "w-3 h-3" : "w-3.5 h-3.5"} />
                  {text("shop_now", "Shop now")}
            ...
            <div className={`great-deals-banner-panel hidden sm:flex flex-col md:flex-row items-center justify-between ${expandedDesktopClass} bg-gradient-to-r from-blue-50 via-white to-sky-50 dark:from-slate-900/70 dark:via-slate-900/60 dark:to-slate-900/70 rounded-2xl shadow-sm w-full relative overflow-hidden border border-slate-200/70 dark:border-slate-800/60`}>
              ...
              <div className={`flex flex-col ${compact ? "gap-1" : "gap-2"} z-10 w-full md:w-auto`}>
                ...
                <span className={`${compact ? "text-[10px] sm:text-xs" : "text-[11px] sm:text-sm md:text-base"} text-blue-800 dark:text-blue-200 font-medium ${compact ? "mb-0.5" : "mb-1.5"}`}>
                  {subtitle}
                </span>
                <span className={`${compact ? "text-[9px] sm:text-[10px]" : "text-[11px] sm:text-sm"} text-blue-700/90 dark:text-blue-300 font-medium`}>
                  {contextLabel}
                </span>
                <button
                  type="button"
                  className={`bg-blue-600 text-white font-semibold ${compact ? "px-3 py-1 text-[10px] sm:text-xs" : "px-3 md:px-5 py-1.5 md:py-2 text-[11px] sm:text-sm md:text-base"} ${compact ? "rounded-full" : "rounded-lg"} shadow hover:bg-blue-700 transition w-fit inline-flex items-center gap-1.5`}
                  onClick={onShopNow}
                >
                  <FaBolt className={compact ? "w-3 h-3" : "w-3.5 h-3.5"} />
                  {text("shop_now", "Shop now")}
