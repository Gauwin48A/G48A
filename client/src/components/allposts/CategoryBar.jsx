...
const AllPostsCategoryBar = ({
  ...
  const wrapperPad = compact ? "px-3 pt-1 pb-1" : "px-3 pt-1.5 pb-1.5";
  const barPad = compact
    ? "py-0.5 px-1.5 md:px-2"
    : "py-1 px-2 md:px-2.5";
  const barGap = compact ? "gap-1 md:gap-1.5" : "gap-1.5 md:gap-2";
  const buttonPad = compact
    ? "px-2 py-1 rounded-full min-w-[44px] sm:min-w-[50px]"
  ...
  return (
    <div className={`w-full flex justify-center ${wrapperPad}`}>
      <div className={`flex flex-col gap-2 ${maxWidthClass} w-full`}>
        ...
        >
          <div className={`mhub-category-track flex items-center ${barGap} w-fit mx-auto`}>
            <button
              type="button"
              onClick={onSelectAll}
              className={`mhub-category-item mhub-chip flex flex-col items-center cursor-pointer transition ${buttonPad} hover:opacity-90 ${
                normalize(activeCategory) === "all"
                  ? "mhub-category-item-active mhub-chip-active"
                  : ""
              }`}
            >
              <span className={`mhub-category-icon ${iconTextSize} mb-0.5`}>
            ...
            {list.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={`mhub-category-item mhub-chip flex flex-col items-center cursor-pointer transition ${buttonPad} whitespace-nowrap hover:opacity-90 ${
                    isActive
                      ? "mhub-category-item-active mhub-chip-active"
                      : ""
                  }`}
                >
                  ...
                  {showCategoryCounts && (
                    <span
                      className={`mhub-category-count text-[9px] sm:text-[10px] font-semibold ${
        ...
        {subcategoryList.length > 0 ? (
          ...
          >
            <div className={`mhub-subcategory-track flex items-center ${barGap} w-fit mx-auto`}>
              <button
                type="button"
                onClick={onSelectAllSubcategories}
                className={`mhub-subcategory-item mhub-chip flex flex-col items-center cursor-pointer transition ${buttonPad} hover:opacity-90 ${
                  normalize(activeSubcategory) === "all"
                    ? "mhub-subcategory-item-active mhub-chip-active"
                    : ""
                }`}
              >
                <span className={`mhub-subcategory-icon ${iconTextSize} mb-0.5`}>
                  <Grid className={iconSize} />
                </span>
                <span
              ...
              {subcategoryList.map((item, index) => {
                ...
                return (
                  ...
                  className={`mhub-subcategory-item mhub-chip flex flex-col items-center cursor-pointer transition ${buttonPad} whitespace-nowrap hover:opacity-90 ${
                    isActive
                      ? "mhub-subcategory-item-active mhub-chip-active"
                      : ""
                  }`}
                >
                    ...
                    {showSubcategoryCounts && (
                      <span
                        className={`mhub-subcategory-count text-[9px] sm:text-[10px] font-semibold ${
                          isActive ? "text-white/90" : "text-slate-500 dark:text-slate-400"
                        }`}
                      >
                        {Number.isFinite(countValue) ? countValue : 0}
