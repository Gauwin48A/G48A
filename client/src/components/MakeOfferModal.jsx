...
const MakeOfferModal = React.memo(({ isOpen, onClose, post, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (parsedPrice < originalPrice * 0.5) {
  ...
  const handleOfferPriceChange = useCallback((e) => {
  ...
  const originalPrice = parsePriceValue(post.price);
  const discount = offerPrice
    ? ((originalPrice - parseFloat(offerPrice)) / originalPrice * 100).toFixed(0)
    : 0;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="mhub-premium-surface rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        {/* Header */}
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                className="mhub-input w-full p-3 rounded-lg text-sm resize-none"
                rows={2}
              />
            </div>
