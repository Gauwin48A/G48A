...
export function mapAuthError(error, options = {}) {
  ...
  // ── Device binding / VPN enforcement errors (403) ────────
  const DEVICE_BINDING_CODES = {
    DEVICE_ALREADY_BOUND: "This device is registered with another account. Only one account per device is allowed.",
    ACCOUNT_SWITCHING_BLOCKED: "This device was used with another account recently. Please wait 24 hours or use your original account.",
    MAX_DEVICES_EXCEEDED: "You have reached the maximum number of allowed devices. Remove an old device from Settings first.",
    LOGIN_COOLDOWN: "Please wait a moment after logging out before logging in again.",
    LOGIN_ABUSE_IP: "Too many login attempts from this network. Please try again later.",
    LOGIN_ABUSE_DEVICE: "Too many login attempts from this device. Please try again later.",
    LOGOUT_ABUSE: "Too many logout requests. Please try again later.",
    VPN_BLOCKED: "VPN or proxy detected. Please disable your VPN to use MHub.",
    TIMEZONE_MISMATCH: "Network anomaly detected. Please disable any VPN or proxy and try again.",
  };

  if (DEVICE_BINDING_CODES[shape.code]) {
    return buildMappedError({
      status: shape.status,
      code: shape.code,
      category: shape.code.startsWith("VPN") || shape.code === "TIMEZONE_MISMATCH" ? "vpn_blocked" : "device_restricted",
      message: DEVICE_BINDING_CODES[shape.code],
      retryable: shape.code === "LOGIN_COOLDOWN",
    });
  }
  ...
  if (shape.status === 401 || shape.status === 403) {
    ...
    return buildMappedError({
      ...
      message: shape.message && !isUnsafeRawMessage(shape.message) ? shape.message : "Sign-in failed. Please check your credentials and retry.",
      retryable: true,
      requiresReauth:
