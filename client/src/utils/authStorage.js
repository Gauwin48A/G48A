 ...
 * Keeps legacy token/user keys compatible while converging on canonical keys.
 */
...
export function getUserId(fallbackUser = null) {
    ...
    const stored = normalizeId(window.localStorage.getItem('userId') || window.localStorage.getItem('user_id'));
    if (stored && !window.localStorage.getItem('userId')) {
...
export function isAuthenticated(fallbackUser = null) {
    return Boolean(getAccessToken() && getUserId(fallbackUser));
}
