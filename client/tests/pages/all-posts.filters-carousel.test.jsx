...
function renderAllPosts(initialEntry = "/all-posts") {
  return render(
...
describe("AllPosts filters and media UX", () => {
  beforeEach(() => {
    ...
    hoisted.categoryMode = {
      ...
      activeApp: null,
      categories: [
        { category_id: "1", name: "Electronics", category_group: "electronics" },
        { category_id: "2", name: "Fashion", category_group: "fashion" },
        { category_id: "3", name: "Vehicles", category_group: "vehicles" },
      ],
      hasSelection: false,
      loading: false,
      subcategories: [],
  ...
  it("loads the selected app immediately when arriving with category_group in the URL", async () => {
    ...
    hoisted.categoryMode = {
      ...hoisted.categoryMode,
      activeApp: "electronics",
    };
    hoisted.apiGet.mockImplementation(async (url) => {
      const parsed = new URL(String(url || ""), "http://localhost");
      if (parsed.searchParams.get("category_group") === "electronics") {
        return {
          posts: [
            {
              ...buildPosts(1)[0],
              post_id: 801,
              title: "Electronics launch post",
    ...
    renderAllPosts("/all-posts?category_group=electronics");

    await waitFor(() => {
      expect(
        hoisted.apiGet.mock.calls.some(([url]) =>
          String(url || "").includes("category_group=electronics"),
        ),
      ).toBe(true);
    }, { timeout: 10000 });
  }, 15000);
  ...
  it("keeps results inside the selected app world even if the response is mixed", async () => {
    hoisted.categoryMode = {
      ...hoisted.categoryMode,
      activeApp: "vehicles",
    };
    ...
    expect(await screen.findByText("Vehicle listing")).toBeTruthy();
    expect(screen.queryByText("Fashion listing")).toBeNull();
  });
});
