...
// Cacheable public read-only endpoints (encoded to avoid exposing route map)
const _ca = ['\x70\x6f\x73\x74\x73','\x63\x61\x74\x65\x67\x6f\x72\x69\x65\x73','\x62\x72\x61\x6e\x64\x73','\x74\x69\x65\x72\x73'];
const CACHEABLE_API = _ca.map(p => new RegExp(`\\/api\\/${p}(\\?|$|\\/)`));

// Sensitive endpoints — never cache (encoded)
const _sa = ['\x61\x75\x74\x68','\x73\x61\x6c\x65','\x70\x61\x79','\x72\x65\x77','\x6e\x6f\x74\x69','\x64\x61\x73\x68','\x66\x65\x65\x64','\x77\x69\x73\x68','\x61\x64\x6d','\x75\x73\x65\x72','\x6f\x66\x66\x65\x72','\x63\x6f\x69\x6e','\x73\x75\x62\x73'];
const SENSITIVE_API = _sa.map(p => new RegExp(`\\/api\\/${p}`));
...
self.addEventListener('fetch', (e) => {
  ...
  if (url.pathname.startsWith('/api/') && SENSITIVE_API.some((r) => r.test(url.pathname))) return; // Never intercept sensitive APIs
