...
For validating sellers securely via Aadhaar OTP and PAN validation endpoints:

- [ ] **`SUREPASS_TOKEN` / `SUREPASS_BEARER_TOKEN`**: The API authorization token provided by Surepass to access Aadhaar OTP and PAN validation endpoints.
- [ ] **`KYC_MODE`**: Change this from `MOCK` (or `development`) to `production`.
  ```env
  KYC_MODE=production
  SUREPASS_API_URL=https://kyc-api.surepass.io/api/v1
  SUREPASS_BEARER_TOKEN=your_surepass_bearer_token
  SUREPASS_TOKEN=your_surepass_bearer_token
