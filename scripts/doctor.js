...
async function run() {
  ...
  const serverRunning = await checkPortInUse(5000);
  const clientRunning = await checkPortInUse(8081) || await checkPortInUse(5173);

  if (serverRunning) warn('Port 5000 is already in use (server may already be running)');
  else ok('Port 5000 is available');
  ...
  if (serverRunning) {
    const healthProbe = await probeHttp('http://127.0.0.1:5000/api/health', {
      headers: { Origin: 'http://localhost:8081' }
    });

    if (healthProbe.error) {
      fail(`Unable to probe http://127.0.0.1:5000/api/health: ${healthProbe.error.message}`);
      failures += 1;
    } else {
      const corsHeader = healthProbe.headers.get('access-control-allow-origin');
      if (corsHeader) ok(`Port 5000 CORS header present for Origin http://localhost:8081 (${corsHeader})`);
      else warn('Port 5000 health response is missing Access-Control-Allow-Origin for http://localhost:8081');
      ...
      if (looksLikeForeignService) {
        fail(`Port 5000 appears to be a different backend service (${healthProbe.json.service}).`);
        failures += 1;
      } else if (looksLikeMhubHealth) {
        ok('Port 5000 appears to be MHub backend (/api/health shape matched).');
      } else {
        warn(`Port 5000 identity unclear. /api/health body: ${healthProbe.bodyText.slice(0, 180)}`);
      }
    }

    const socketProbe = await probeHttp('http://127.0.0.1:5000/socket.io/?EIO=4&transport=polling', {
      headers: { Origin: 'http://localhost:8081' }
    });
