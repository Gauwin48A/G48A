...
const testcaseCatalogJsonPath = path.join(rootDir, 'server', 'docs', 'TESTCASE_CATALOG.json');
const featureMatrixPath = path.join(rootDir, 'server', 'docs', 'project', 'FEATURE_COMPLETION_MATRIX.md');
...
function loadTestcaseCatalog() {
  if (!fs.existsSync(testcaseCatalogJsonPath)) {
    throw new Error(`Missing test case catalog JSON: ${testcaseCatalogJsonPath}`);
  }

  const raw = fs.readFileSync(testcaseCatalogJsonPath, 'utf8');
  const parsed = JSON.parse(raw);
  const textBlock =
    typeof parsed?.catalogText === 'string' && parsed.catalogText.trim().length > 0
      ? parsed.catalogText
      : '';
  const referencesBlock = Array.isArray(parsed?.references)
    ? parsed.references
  ...
  const merged = [textBlock, referencesBlock].filter(Boolean).join('\n');
  if (merged.trim().length > 0) {
    return merged;
  }

  throw new Error(
    `Invalid test case catalog JSON format at ${testcaseCatalogJsonPath}. Expected catalogText or references.`,
  );
}
...
function main() {
  ...
  const testcaseCatalog = loadTestcaseCatalog();
