...
const checks = [
  {
    ...
    cwd: rootDir,
    args: ['server/scripts/workspace/guard-tracked-deps.js']
  },
  {
    name: 'Workspace footprint guard',
    command: process.execPath,
    cwd: rootDir,
    args: ['server/scripts/workspace/guard-footprint.js']
  },
  {
    name: 'Workspace codebase line budget guard',
    command: process.execPath,
    cwd: rootDir,
    args: ['server/scripts/workspace/line-budget.js', '--mode=guard']
  },
  {
    name: 'Workspace locale validation',
    command: process.execPath,
    cwd: rootDir,
    args: ['server/scripts/workspace/validate-locales.js']
  },
  {
    name: 'Proactive workflow contract gate',
    command: process.execPath,
    cwd: rootDir,
    args: ['server/scripts/workspace/check-proactive-workflow-contract.js']
  },
  ...
  npmStep('Client performance budget gate', path.join(rootDir, 'client'), ['run', 'check:performance-budget']),
