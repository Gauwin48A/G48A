...
-- Login: rahul.sharma@mhub.com / Password123!
-- =====================================================
...
-- USERS (with ALL columns required by authController.js)
CREATE TABLE users (
...
-- (User Verifications moved to later section)
...
-- ADVANCED INDEXES (Operation Scale-Up)
-- 1. Full-Text Search Speed (GIN)
...
CREATE INDEX idx_recently_viewed_source ON recently_viewed(user_id, source);
...
-- ===========================================
-- STEP 11: FORTRESS SCHEMA & ANALYTICS (ADDED)
-- ===========================================
...
CREATE INDEX IF NOT EXISTS idx_post_metrics_promoted 
...
BEGIN
    ...
    SET 
        engagement_rate = CASE 
            ...
            THEN ROUND((COALESCE(NEW.likes, 0)::decimal / NEW.views_count), 4)
            ELSE 0 
        END,
        last_updated = NOW(),
...
AFTER UPDATE OF views_count, likes ON posts
FOR EACH ROW
...
SELECT 
    ...
    CASE WHEN COALESCE(views_count, 0) > 0 
         THEN ROUND((COALESCE(likes, 0)::decimal / views_count), 4)
         ELSE 0 
    END,
...
RETURNS TABLE (
    post_id INT, title VARCHAR, description TEXT, price DECIMAL, 
    distance_km FLOAT, location VARCHAR, latitude FLOAT, longitude FLOAT, 
    user_id INT, category_id INT, status VARCHAR, created_at TIMESTAMP
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        p.post_id::INT, p.title::VARCHAR, p.description::TEXT, p.price,
        (6371 * acos(LEAST(1.0, GREATEST(-1.0,
            cos(radians(input_lat)) * cos(radians(p.latitude)) * 
            cos(radians(p.longitude) - radians(input_long)) + 
            sin(radians(input_lat)) * sin(radians(p.latitude))
        ))))::FLOAT AS distance_km,
        p.location::VARCHAR, p.latitude::FLOAT, p.longitude::FLOAT,
        p.user_id::INT, p.category_id::INT, p.status::VARCHAR, p.created_at::TIMESTAMP
    FROM posts p
...
-- 4. FEATURE: USER TRUST SCORE SYSTEM
...
SELECT 
    ...
    (
        ...
        LEAST(50, COALESCE((SELECT COUNT(*) * 5 FROM transactions t WHERE (t.buyer_id = u.user_id OR t.seller_id = u.user_id) AND t.status = 'completed'), 0)) +
        LEAST(20, COALESCE((SELECT points / 100 FROM rewards r WHERE r.user_id = u.user_id), 0))
    ) as trust_score
FROM users u
LEFT JOIN profiles p ON u.user_id = p.user_id
LEFT JOIN rewards r ON u.user_id = r.user_id;

-- 5. FEATURE: AUTH SYSTEM UPGRADES
...
SELECT 'Admin: rajesh.verma@mhub.com / Password123!' as admin;
SELECT '=============================================' as separator;

-- ===========================================
-- STEP FINAL: UPDATE STATISTICS FOR PERFORMANCE
-- ===========================================
ANALYZE users;
ANALYZE profiles;
ANALYZE posts;
...
SELECT '✅ MHUB ULTIMATE v6.0 - Database Ready!' as final_status;
