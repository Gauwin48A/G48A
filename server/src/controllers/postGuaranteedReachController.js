...
const logInfo =
  logger && logger.info ? logger.info : console.log;

const {
  GUARANTEED_REACH_QUERY,
  GUARANTEED_REACH_FALLBACK,
} = require("../queries/guaranteedReachQuery");
...
async function getFromCache(key) {
  try {
    ...
    if (data !== null && data !== undefined) {
      const isRedis =
        typeof redisCache.isRedisAvailable === "function"
    ...
    logInfo(
      "[GuaranteedReach] Cache get error, continuing without cache"
    );
    return null;
  }
}
...
/**
 * Fetch guaranteed-reach posts with filtering, sorting, pagination, and caching.
 * Returns a fair-feed of posts using the guaranteed-reach algorithm, falling
 * back to a simpler query on timeout or empty results.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
exports.getGuaranteedReachPosts = async (req, res) => {
  try {
    ...
    if (rows) {
      fromCache = true;
      logInfo(
        `[GuaranteedReach] REDIS CACHE HIT - ${cacheKey.substring(0, 30)}...`
      );
    } else {
      try {
        const result = await queryWithTimeout(GUARANTEED_REACH_QUERY, [
          userId,
          queryLimit,
          refreshSeed,
        ]);
        ...
        logInfo(
          `[GuaranteedReach] CACHE MISS - Query: ${Date.now() - startTime}ms, ${rows.length} posts`
        );
      } catch (queryErr) {
        logInfo("[GuaranteedReach] Timeout, using fallback query");
        const fallback = await queryWithTimeout(GUARANTEED_REACH_FALLBACK, [
          queryLimit,
          userId,
        ]);
        rows = fallback.rows;
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      logInfo(
        "[GuaranteedReach] Empty fair-feed result, using fallback feed query"
      );
      const fallback = await queryWithTimeout(GUARANTEED_REACH_FALLBACK, [
        queryLimit,
        userId,
    ...
    const enrichedPosts = await attachTrustToPosts(transformedPosts);
