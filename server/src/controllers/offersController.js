...
/* ── Schema detection (cached promise — runs once) ─────────────── */

let _updatedAtAvailable = null;
let _updatedAtMissingLogged = false;

function hasOffersUpdatedAtColumn() {
  if (!_updatedAtAvailable) {
    _updatedAtAvailable = runQuery(`
      SELECT EXISTS (
        SELECT 1
  ...
  return _updatedAtAvailable;
}

function getOfferUpdatedAtSelect(hasUpdatedAt, alias = "") {
  const prefix = alias ? `${alias}.` : "";
  return hasUpdatedAt ? `${prefix}updated_at` : `${prefix}created_at AS updated_at`;
}

function getOfferUpdatedAtSetClause(hasUpdatedAt) {
  return hasUpdatedAt ? ", updated_at = NOW()" : "";
}
  ...
  try {
    ...
    const result = await runQuery(
      `INSERT INTO offers (post_id, buyer_id, seller_id, offered_price, original_price, message)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING
         offer_id, post_id, buyer_id, seller_id,
         offered_price, original_price, message, status, counter_price,
         created_at, ${getOfferUpdatedAtSelect(offersHasUpdatedAt)}`,
      [postId, buyerId, sellerId, offeredPrice, post.price, message],
    );

    return res.status(201).json({ message: "Offer sent successfully", offer: result.rows[0] });
  } catch (error) {
...
/* ── GET /api/offers — List offers for the authenticated user ──── */
  ...
  try {
    ...
    let query = `
      SELECT
        ...
        o.offer_id, o.post_id, o.buyer_id, o.seller_id,
        o.offered_price, o.original_price, o.message,
...
/* ── PUT /api/offers/:offerId — Accept / Reject / Counter ──────── */
...
const respondToOffer = async (req, res) => {
  ...
  try {
    ...
    // Auto-mark post as sold on acceptance
    if (action === "accept") {
      await client.query(
        "UPDATE posts SET status = 'sold', updated_at = NOW() WHERE post_id = $1",
        [offer.post_id],
      );
    }

    await client.query("COMMIT");
    return res.json({ message: `Offer ${newStatus}`, offer: updatedOffer.rows[0] });
  } catch (error) {
    await client.query("ROLLBACK");
...
/* ── GET /api/offers/history/:postId — Offer history for a post ── */
...
/* ── POST /api/offers/auto-accept — Set auto-accept threshold ──── */
...
const setAutoAcceptThreshold = async (req, res) => {
  ...
  try {
    ...
    const post = await client.query(
      "SELECT user_id FROM posts WHERE post_id = $1 FOR UPDATE",
      [postId],
    );
    ...
    await client.query(
      "UPDATE posts SET auto_accept_price = $1, updated_at = NOW() WHERE post_id = $2",
      [minAcceptPrice, postId],
    );

    // Auto-accept any pending offers at or above the threshold
...
/* ── CRON — Expire stale pending offers (>48 h) ───────────────── */
