...
/**
 * Parse a value to integer with fallback.
 * @param {*} value
...
/**
 * Parse a value to float with fallback.
 * @param {*} value
 * @param {number} fallback
 * @returns {number}
 */
...
/**
 * Fetch aggregated seller analytics (posts, inquiries, offers, revenue, reviews).
 * Caches the result and upserts into seller_analytics table.
        ...
        return {
          overview: {
            ...
            avgRating: data.avg_rating
              ? parseFloatValue(data.avg_rating, 0).toFixed(1)
              : null,
...
/**
 * Fetch per-post performance metrics (views, inquiries, offers) for the
 * authenticated seller. Returns the top 20 posts by view count.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
...
/**
 * Fetch category breakdown for the authenticated seller showing post counts,
 * total views, and sold counts grouped by category.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
const getCategoryBreakdown = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch category breakdown" });
  }
};

/**
 * Persist client device fingerprint and metadata for analytics.
 * Inserts a row into device_analytics and invalidates the summary cache.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
const saveDeviceInfo = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, id: result.rows[0].id, fingerprint });
  } catch (error) {
    logger.error("[Analytics] Device save error:", error);
    return res
      .status(200)
      .json({ success: false, error: "Analytics temporarily unavailable" });
  }
};

/**
 * Capture a client-side error and forward it to the error reporter service.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
const saveClientError = async (req, res) => {
  try {
    ...
    const normalizedPayload = {
      message: message,
      stack: String(payload.stack || "").slice(0, 5e3),
...
/**
 * Capture a client-side UX telemetry event and forward it to the error
 * reporter as a tagged message for centralized collection.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
const saveClientEvent = async (req, res) => {
  try {
    ...
    const eventName = String(payload.eventName || payload.event || "")
      .trim()
      .slice(0, 120);
    if (!eventName) {
...
/**
 * Return a cached summary of device analytics from the last 30 days,
 * grouped by device type, browser, and OS.
 * @param {import("express").Request} req
 * @param {import("express").Response} res
 */
  ...
  try {
    const payload = await cacheService.getOrSetWithStampedeProtection(
      ...
      async () => {
        ...
        return { summary: summary.rows, period: "30 days" };
      },
