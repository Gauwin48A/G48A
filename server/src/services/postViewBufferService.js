...
const DEFAULT_BATCH_LIMIT = Number.parseInt(process.env.BATCH_VIEW_MAX_IDS || '500', 10);
const DEFAULT_FLUSH_INTERVAL_MS = Number.parseInt(process.env.BATCH_VIEW_FLUSH_INTERVAL_MS || '250', 10);
const DEFAULT_MAX_BUFFERED_KEYS = Number.parseInt(process.env.BATCH_VIEW_MAX_BUFFERED_KEYS || '5000', 10);
...
function mergeBackCounts(countMap) {
    for (const [postId, increment] of countMap.entries()) {
        ...
        if (pendingCounts.size >= DEFAULT_MAX_BUFFERED_KEYS && !pendingCounts.has(postId)) {
...
async function enqueueBatchView(postIds, options = {}) {
    const syncMode = options.syncMode === true || process.env.BATCH_VIEW_SYNC_MODE === 'true';
    const sanitized = sanitizePostIds(postIds, options.limit || DEFAULT_BATCH_LIMIT);
    ...
    if (syncMode) {
        ...
        return {
            ...
            skipped: Math.max(0, (Array.isArray(postIds) ? postIds.length : sanitized.length) - sanitized.length),
    ...
    return {
        mode: 'async',
        queued,
        skipped: Math.max(0, (Array.isArray(postIds) ? postIds.length : queued) - queued),
        updated: 0,
        flushScheduled
    };
}
