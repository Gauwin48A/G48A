 ...
 * OTP Service
 *
 * Generate, store, verify, and deliver one-time passwords via SMS
...
/**
 * Generate an OTP, invalidate previous unused OTPs for the same
 * user/purpose, and persist the new hashed OTP to otp_store.
 * @param {string} userId
 * @param {string} [purpose="sale_confirm"]
 * @returns {Promise<string>} The plaintext OTP (for delivery).
 */
...
/**
 * Verify an OTP code against the most recent unused record for a user/purpose.
 * Handles expiry checks and attempt limiting.
 * @param {string} userId
 * @param {string} code - The plaintext OTP to verify.
 * @param {string} [purpose="sale_confirm"]
...
const verifyOTP = async (userId, code, purpose = "sale_confirm") => {
  ...
  try {
    const result = await pool.query(
      `SELECT
         id, user_id, otp_hash, purpose,
         expires_at, attempts, is_used, created_at
       FROM otp_store
       WHERE user_id = $1 AND purpose = $2 AND is_used = false
    ...
    await pool.query(
      "UPDATE otp_store SET attempts = attempts + 1 WHERE id = $1",
      [rec.id]
    );
...
/**
 * Send an OTP via the specified channel (sms or email).
 * Tries configured providers in order, falling back to mock output.
 ...
 * @returns {Promise<object>} Delivery result with provider info.
 */
const sendOTP = async (channel, destination, otp, options = {}) => {
  ...
  /**
   * Finalize the delivery record and merge provider details into result.
   */
  const finalizeAndReturn = async (result, statusOverride = null) => {
    const provider = result?.provider || null;
    const providerMessageId =
      result?.providerMessageId || result?.messageId || result?.sid || null;
    const sendStatus =
      statusOverride ||
      (result?.mock ? "mock" : result?.success ? "sent" : "failed");
    ...
    await otpDeliveryService.finalizeDeliveryRecord({
      ...
      providerMessageId,
      sendStatus,
      metadata: { ...metadata, flow, purpose },
    });

    return { ...result, deliveryId, providerMessageId };
  };
  ...
  return finalizeAndReturn({ success: true, mock: true, provider: "mock" }, "mock");
};
