...
ranked_posts AS (
    SELECT
        a.*,
        -- Total score = promotion + tier + random + engagement + freshness
        (a.boost_boost + a.tier_boost + a.random_score + a.engagement_boost + a.freshness_boost) AS total_score,
        -- Author diversity: Only 1 post per author
        ROW_NUMBER() OVER (PARTITION BY a.author_id::text ORDER BY a.boost_boost DESC, a.tier_boost DESC, a.freshness_boost DESC, a.random_score DESC) AS author_rank,
        -- Category diversity: Priority within category
        ROW_NUMBER() OVER (PARTITION BY a.category_id::text ORDER BY a.boost_boost DESC, a.tier_boost DESC, a.freshness_boost DESC, a.random_score DESC) AS category_rank
...
ORDER BY
    ...
    r.boost_boost DESC,
    -- TIER PRIORITY NEXT (Premium > Silver > Bronze > Basic)
 ...
 *
 * Parameters: $1 = uid (text), $2 = limit (int).
 * @type {string}
 */
