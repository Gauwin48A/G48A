...
/**
 * Parse a positive float, clamped between min and max.
 */
...
/**
 * Parse a positive integer, clamped between min and max.
 */
...
/**
 * @route Nearby routes
 * @description Location-based post discovery and city aggregation
 */

/** @route GET / - Find posts near a given lat/long within a radius */
router.get("/", async (req, res) => {
  try {
    ...
    const latParam = lat || latAlias;
    const longParam = long || lng || lngAlias;
    ...
    const longitude = Number.parseFloat(longParam);
    ...
    const lngDelta = Math.abs(cosLat) > 1e-5
      ? radiusKm / (111.32 * Math.abs(cosLat))
      : 180;
...
function formatDistance(km) {
  if (km < 1) {
    return `${Math.round(km * 1e3)}m away`;
  } else if (km < 10) {
...
/** @route GET /cities - Get top 20 cities with active post counts */
