...
/**
 * Execute a query with a configured timeout.
 */
...
/**
 * Parse a value as a positive integer with an optional ceiling.
 */
...
/**
 * Normalize an optional string, returning null when empty.
 */
...
const createChannelHandler = async (req, res) => {
  try {
    ...
    if (!verified)
      return res.status(403).json({ error: "Aadhaar verification required" });
