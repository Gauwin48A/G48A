...
// The old sticky header block (lines 1633-1750)
...
if (idx < 0) {
    ...
    // Debug: search for partial patterns
    ...
    if (sfIdx >= 0) {
        ...
        console.log(`Context: ${content.substring(sfIdx, sfIdx + 200)}`);
    } else {
        console.log('Could not find stickyHeader at all');
        // Find any occurrence of stickyHeader
        const allOccurrences = content.match(/stickyHeader/g);
        console.log(`All 'stickyHeader' occurrences: ${allOccurrences?.length || 0}`);
    }
    process.exit(1);
}
...
console.log(`SUCCESS: Sticky header replaced. Lines: ${content.split('\\n').length}. Braces: ${opens}/${closes} (diff=${opens - closes}).`);
