...
- **Scroll height:** 1866px (NaN screens)
- **Score:** **8.4/10**
...
### Tiny fonts (<12px)
- `<span>` 11px — "Home" — `mhub-bottom-nav-label is-active`
- `<span>` 11px — "All Posts" — `mhub-bottom-nav-label `
- `<span>` 11px — "Sell" — `mhub-bottom-nav-label `
- `<span>` 11px — "Chat" — `mhub-bottom-nav-label `
- `<span>` 11px — "Profile" — `mhub-bottom-nav-label `
- `<p>` 11px — "TRUST-FIRST MARKETPLACE" — `home-hero-kicker`
- `<span>` 11px — "⚡ Live" — `home-hero-live-badge`
- `<p>` 11.2px — "Fresh listings, verified sellers, and qu" — `home-hero-sub`
- `<span>` 11px — "Browse All" — `home-action-label`
- `<span>` 11px — "For You" — `home-action-label`
- `<span>` 11px — "Feed" — `home-action-label`
- `<span>` 11px — "Rewards" — `home-action-label`
- `<span>` 11px — "📦 8 listings" — `home-stat`
- `<span>` 11px — "🏷️ 2 categories" — `home-stat`
- `<span>` 11px — "✅ Verified sellers" — `home-stat`
- `<button>` 11px — "All (8)" — `home-category-chip active`
- `<button>` 11px — "Vehicles (4)" — `home-category-chip `
- `<button>` 11px — "Fashion (4)" — `home-category-chip `
- `<span>` 11px — "8 items" — `home-results-count`
- `<p>` 11px — "Bangalore, Karnataka" — `mhub-grid-card-location`
- ...+11 more

### Small touch targets (<36×36)
- `<button>` 32×36 — "↻" — `home-refresh-btn`
...
- **log**: [MHub:Response] OK 200 /auth/session
- **log**: i18next::translator: missingKey en-US translation reload Reload
- **warning**: Service Worker registration blocked by Playwright
- **log**: [MHub:Response] OK 200 /categories/with-subcategories
- **log**: [MHub:Interceptor] START /posts

### Screenshots
- Full page: `home-full.png`
- `home-scroll00.png`
- `home-scroll01.png`
- `home-scroll02.png`